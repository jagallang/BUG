...
extension ResponsiveText on num {
  double get rsp => kIsWeb ? (this * 1.1).toDouble() : this.sp;
  double get rw => kIsWeb ? (this * 0.9).w : this.w;
  double get rh => kIsWeb ? (this * 0.9).h : this.h;
  double get rr => kIsWeb ? (this * 0.9).r : this.r;
}
...
void main() async {
  ...
  try {
    ...
    if (kDebugMode) {
      ...
      // 데모 데이터 초기화 (최초 실행시에만) - 타임아웃 5초
      try {
        await FirebaseService.initializeDemoData().timeout(
          const Duration(seconds: 5),
          onTimeout: () {
            if (kDebugMode) {
              print('Demo data initialization timed out - continuing without demo data');
            }
          },
        );
      } catch (demoError) {
        if (kDebugMode) {
          print('Demo data initialization failed: $demoError');
        }
      }
    }
  } catch (e) {
    if (kDebugMode) {
      ...
      print('Running in offline mode with fallback data');
    }
  }
